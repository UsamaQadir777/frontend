...
export default function AppNavigator() {
  ...
  const loadAuthenticatedProfile = async () => {
    const [data, user] = await Promise.all([fetchProfile(), fetchCurrentUser()]);
    const nextProfile = normalizeProfileFromApi(data, user);
  ...
  const value = useMemo(
    () => ({
      ...
      completeOnboarding: async (nextProfile) => {
        ...
        setProfile((currentProfile) => {
          const savedProfile = normalizeProfileFromApi(
            { ...currentProfile, ...nextProfile, ...data, onboarding_completed: true },
            currentUser
          );
    ...
    [currentUser, dashboardRefreshKey, isDark, meals, profile, themeMode]
  );
